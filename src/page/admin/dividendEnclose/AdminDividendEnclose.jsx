...
import AdminAddDividendEncloseModal from "./components/AdminAddDividendEncloseModal";
import { useGetAdminDividendAncloseQuery, useDeleteAdminDividendAncloseMutation } from "../../../features/admin/dividendEnclose/adminDividendEncloseApiSlice";

function AdminDividendEnclose() {
  /* State */
  const [openAddExpensesModal, setOpenAddExpensesModal] = useState(false);

  /* API */
  const { data, isLoading, isError, refetch } = useGetAdminDividendAncloseQuery();
  const [deleteDividend] = useDeleteAdminDividendAncloseMutation();
  ...
  const handleDeleteItem = async (id) => {
    ...
    try {
      const resData = await deleteDividend(id).unwrap();
...
export default AdminDividendEnclose;
