...
function CashierAddExpenseModal() {
  /* Form */
  ...
  /* API */
  const { data, isLoading } = useGetCashierExpensesCategoryQuery();
  const [addExpense] = useAddCashierExpensesMutation();
  ...
  const handleSubmit = async (values) => {
    const { expensesCategory, naqdsum, naqdusd, valyuta, bank, karta, izoh } =
      values;
    /* Set Event */
    ...
    try {
      ...
      const resData = await addExpense(data).unwrap();
...
export default CashierAddExpenseModal;
